...
/**
 * This application demonstrates how a Charva application is structured and how
 * each of the Charva features works.
 */
public class CharvaExample extends JFrame implements ActionListener {
	public CharvaExample() {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source instanceof JMenuItem) {
			...
			_textarea.append("JTextField contains \"" + _textfield.getText() + "\"\n");
		} else if (source == _combobox) {
			_textarea.append(_combobox.getSelectedItem() + " was selected in JComboBox\n");
		} else if (source == _radiobutton) {
			_textarea.append("JRadioButton is ");
			String msg = _radiobutton.isSelected() ? "selected\n" : "deselected\n";
			_textarea.append(msg);
		} else if (source == _checkbox) {
			_textarea.append("JCheckBox is ");
			String msg = _checkbox.isSelected() ? "selected\n" : "deselected\n";
			_textarea.append(msg);
		}
	}
...
class GridBagTest extends JDialog implements ActionListener, ListSelectionListener {
	...
	public GridBagTest() {
		...
		_style = new JList(new String[] { "Serif", "SansSerif", "Monospaced", "Dialog", "DialogInput" });
	...
	private void add(Component c_, GridBagConstraints gbc_, int x_, int y_, int w_, int h_) {
		gbc_.gridx = x_;
		gbc_.gridy = y_;
		gbc_.gridwidth = w_;
		gbc_.gridheight = h_;
	...
	private void updateFont() {
		Font font = new Font((String) _style.getSelectedValue(), (_bold.isSelected() ? Font.BOLD : 0) + (_italic.isSelected() ? Font.ITALIC : 0), Integer.parseInt(_size.getText()));
