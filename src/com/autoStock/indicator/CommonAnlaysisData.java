...
public class CommonAnlaysisData {
	...
	private double[] extractDoubleFromQuoteSlice(ArrayList<QuoteSlice> listOfQuoteSlice, String field){
		...
		for (QuoteSlice quoteSlice : listOfQuoteSlice){
			...
			else if (field.equals("priceHigh")){arrayOfDouble[i] = quoteSlice.priceHigh;}
			else if (field.equals("priceLow")){arrayOfDouble[i] = quoteSlice.priceLow;}
			else if (field.equals("priceClose")){arrayOfDouble[i] = quoteSlice.priceClose;}
			i++;
		}
